...
// Query to fetch enemies from the `enemy` table
$sql = "SELECT id, name FROM enemy";
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    // Use these IDs to perform further actions like fetching enemy stats from the database
    // Example: Fetching details of the selected enemies
    $sql = "SELECT * FROM enemy WHERE id IN ($enemy1_id, $enemy2_id, $enemy3_id)";
    $result = mysqli_query($conn, $sql);
}
?>
            ...
            <select name="enemy1"
                ...
                <?php foreach ($enemies as $enemy): ?>
                    <option value="<?php echo $enemy['id']; ?>">
                        <?php echo htmlspecialchars($enemy['name']); ?>
                    </option>
                <?php endforeach; ?>
            </select>
