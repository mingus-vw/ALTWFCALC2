...
// Handle form submissions for adding, deleting, favoriting, and unfavoriting enemies
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['add_enemy'])) {
        ...
        // Check if the enemy is favored
        $stmt = $db->prepare("SELECT is_favored FROM enemy WHERE id = ?");
        $stmt->execute([$enemy_id]);
        $enemy = $stmt->fetch();

        if ($enemy && !$enemy['is_favored']) {
    ...
    } elseif (isset($_POST['favorite_enemy'])) {
        // Favorite enemy
        $enemy_id = $_POST['enemy_id'];
        $stmt = $db->prepare("UPDATE enemy SET is_favored = TRUE WHERE id = ?");
        $stmt->execute([$enemy_id]);
    } elseif (isset($_POST['unfavorite_enemy'])) {
        // Unfavorite enemy
        $enemy_id = $_POST['enemy_id'];
        $stmt = $db->prepare("UPDATE enemy SET is_favored = FALSE WHERE id = ?");
        $stmt->execute([$enemy_id]);
    }
}
...
$enemies_stmt = $db->query("
    SELECT enemy.id, enemy.name, enemy.stats, faction.name AS faction_name, enemy.is_favored
    FROM enemy
